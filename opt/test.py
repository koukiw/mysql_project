from get_text import excel2text
import glob
import datetime
import pytz
import pandas as pd
...
def get_text(project_name):
    try:
        if project_name =="":
            ...
            files = glob.glob(os.path.join("./file_dir", project_name,"**","*.xlsx"),recursive=True)
        print(project_name)
        cnt = 0
        data = []
        print(files)
...
#     print(df_tmp)
    # outpudt_df = pd.concat([output_df,df_tmp],ignore_index=True)
